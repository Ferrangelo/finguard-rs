                  ...
                  <tr key={cat} className="hover:bg-muted/20">
                    ...
                    {months.map((m) => (
                      <td key={m} className="px-1 py-1">
                        <MathInput value={income[m]?.[cat] ?? 0} onCommit={(v) => setCell(m, cat, v)} />
                      </td>
                    ))}
                    <td className="px-3 py-1.5 text-right font-semibold tabular-nums">{formatRef(total)}</td>
                  </tr>
                );
              })}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <GlassCard title="Income distribution">
          <div className="h-80">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={incomePie} dataKey="value" nameKey="name" innerRadius={55} outerRadius={110} paddingAngle={2}>
                  {incomePie.map((_, i) => <Cell key={i} fill={colorAt(i)} stroke="oklch(0.16 0.02 265)" />)}
                </Pie>
...
function DerivedRow({
  label, values, variant,
}: { label: string; values: number[]; variant: "strong" | "positive" | "negative" | "percent" }) {
  const total = variant === "percent"
  ...
  const cls =
    ...
    variant === "percent" ? "text-warning" : "text-foreground";
  const bg =
    variant === "positive" ? "bg-success/5" :
    variant === "negative" ? "bg-destructive/5" :
    variant === "percent" ? "bg-warning/5" : "bg-muted/30";
  return (
    <tr className={`${bg} font-semibold`}>
      <td className={`px-3 py-1.5 ${cls}`}>{label}</td>
      {values.map((v, i) => (
        <td key={i} className={`px-2 py-1.5 text-right tabular-nums ${cls}`}>{fmt(v)}</td>
      ))}
