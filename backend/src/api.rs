//! Finguard Axum HTTP surface: routes, DTOs, and handlers.
//!
...
//!
//! This module is the HTTP surface only: it defines the route table (see
//! [`router`]), decodes and validates request bodies/query strings into the
//! DTOs below, and translates domain errors ([`crate::Error`], via
//! `AppError`) into HTTP status codes. All persistence
//! and business logic live one layer down in [`crate::df_operations`]
//! (Parquet-backed dataframes) and [`crate::config`] (JSON category config);
//! handlers here should stay thin wrappers around those types.
//!
//! The `.route(...)` calls in [`router`] are the single source of truth for
//! the API surface. Grep for `.route(` rather than trusting a cached copy of
//! the route list in documentation, including `PROJECT_SUMMARY.md`.
//!
//! The `finguard_rs_backend` binary embeds [`router`] after binding its
//! listener and running the row ID migration. Any other embedder, such as a
...
fn str_col_to_vec(df: &polars::prelude::DataFrame, name: &str) -> crate::Result<Vec<String>> {
    Ok(df
        .column(name)?
        .str()?
        .iter()
...
// ======================================================================
//
// Grouped below by resource, matching the route groups in [`router`]. Each
...
// resulting [`crate::Error`] to an HTTP status through
...
fn discover_years() -> Vec<i32> {
    let Ok(root) = crate::paths::get_dbs_root() else {
        return vec![];
    };
...
/// pair is [`crate::df_operations::resolve_fact`]'s only
...
fn resolve_fact_lenient(
    fact: &crate::df_operations::ExpenseFact,
    reference_currency: &str,
    rates: &std::collections::HashMap<(chrono::NaiveDate, String), fx::ResolvedRate>,
) -> Option<(f64, chrono::NaiveDate)> {
    crate::df_operations::resolve_fact(fact, reference_currency, rates).ok()
}
...
/// verbatim.
///
/// [`Error::RowIdsMissing`]: crate::Error::RowIdsMissing
///
...
async fn get_expenses_handler(
    ...
    let mut rows: Vec<(ExpenseJson, crate::df_operations::ExpenseFact)> = Vec::new();
    ...
    for m in months {
        let de = match DetailedExpenses::new(q.year, m) {
            Ok(de) => de,
            Err(err @ crate::Error::RowIdsMissing(_)) => return Err(err.into()),
            Err(_) => continue,
        };
        ...
        for (fi, row_id) in row_ids.into_iter().enumerate() {
            ...
            rows.push((
                ...
                crate::df_operations::ExpenseFact {
                    expense_date,
                    currency,
...
async fn upsert_expense_handler(
    ...
    let reference_currency = config::get_currency_settings()?.reference_currency;
    let fact = crate::df_operations::ExpenseFact {
        expense_date,
    ...
    let keys =
        crate::df_operations::distinct_rate_keys(std::slice::from_ref(&fact), &reference_currency);
...
/// `"secondary"`.
///
/// Reads raw facts (see [`crate::df_operations::all_expense_facts`])
...
async fn category_totals_across_all_years(kind: &str) -> Result<CategoryTotalsJson, AppError> {
    ...
    let facts = crate::df_operations::all_expense_facts()?;
    let reference_currency = config::get_currency_settings()?.reference_currency;
    let keys = crate::df_operations::distinct_rate_keys(&facts, &reference_currency);
    let (rates, unavailable_currencies) = fx::rates_for_keys_lenient(&keys).await?;
...
mod tests {
    ...
    /// docs on `crate::fx`), since the cache's `RateCache` type and
    /// (de)serialization helpers are private to that module.
    fn seed_fx_cache(dates: &[(&str, &[(&str, f64)])]) {
        ...
        let path = crate::paths::get_fx_rates_path().expect("fx rates path");
    ...
    /// pins the behavior `upsert_expense_handler`'s docs describe, independent
    ...
    async fn delete_category_refuses_while_its_own_currency_cannot_be_resolved() {
        ...
        let crate::Error::InvalidArgument(message) = err.0 else {
    ...
    async fn currency_settings_update_rejects_an_unsupported_reference_currency() {
        ...
        assert!(matches!(err.0, crate::Error::InvalidArgument(_)));
    ...
    async fn handlers_reject_a_file_without_row_ids() {
        ...
        for (path, df) in [
            ...
            (
                crate::paths::get_year_summary_path(
                    2026,
                    crate::paths::RECURRING_EXPENSES_FILENAME,
                )
                .unwrap(),
                rec.df.clone(),
            ),
        ...
        let assert_restart_error = |err: AppError| {
            let crate::Error::RowIdsMissing(message) = &err.0 else {
