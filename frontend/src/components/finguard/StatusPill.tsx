...
export function StatusPill() {
  ...
  return (
    <div
      className={`inline-flex items-start gap-2 rounded-2xl border px-3 py-1 text-xs font-medium transition-colors max-w-md ${color}`}
    >
      <Icon
        className={`h-3.5 w-3.5 mt-px shrink-0 ${status.kind === "loading" ? "animate-spin" : ""}`}
      />
